...
class ModelNetDataLoader(Dataset):
    def __init__(self, root, args, ind, split='train', process_data=False):
        ...
        elif self.num_category == 4:
            self.catfile = os.path.join(self.root, 'me_shape_name4.txt')
        else:
            self.catfile = os.path.join(self.root, 'me_shape_name3.txt')
        ...
        if self.num_category == 10:
            ...
            shape_ids['train'] = [micro_data[key].split('/')[1][:-4] for key in micro_data if key.split('/')[0] not in ind]  #list(np.random.choice(shape_ids_raw['train'], int(len(shape_ids_raw['train'])* 0.8), replace=False)) + 
            shape_ids['test'] = [micro_data[key].split('/')[1][:-4] for key in micro_data if key.split('/')[0] in ind] 
        elif self.num_category == 4:
            with open(os.path.join(self.root,'micro_cropped_exp_4cls.json'),'r',encoding='utf8')as fp:
                micro_data = json.load(fp)
            ...
            with open(os.path.join(self.root,'micro_cropped_exp_3cls.json'),'r',encoding='utf8')as fp:
                micro_data = json.load(fp)
            with open(os.path.join(self.root,'macro_cropped_exp_3cls.json'),'r',encoding='utf8')as fp:
                macro_data = json.load(fp)
            shape_ids_raw['train'] = [macro_data[key].split('/')[1][:-4] for key in macro_data]
            shape_ids_raw['test'] = [micro_data[key].split('/')[1][:-4] for key in micro_data]
            
            shape_ids['train'] = [micro_data[key].split('/')[1][:-4] for key in micro_data if key.split('/')[0] not in ind]  # list(np.random.choice(shape_ids_raw['train'], int(len(shape_ids_raw['train'])* 1), replace=False)) + 
            shape_ids['test'] = [micro_data[key].split('/')[1][:-4] for key in micro_data if key.split('/')[0] in ind] 
